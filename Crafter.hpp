...
class Crafter {
public:
	Crafter(std::vector<Skills::SkillName> startingMoves, int maxCP, int maxProgress, int maxQuality, int maxDurability, bool topQuality) {
	...
	void ForceCraft() {
		...
		for (const auto& move : player->fullSkillList) {
			...
			if ((currentTime + 3) == bestTime) {		// Only one move left to match the best time
			...
			if (!topQuality || player->craftableItem->IsItemMaxQuality()) {
				if (move == SkillName::FINALAPPRAISAL)		continue;
			}
